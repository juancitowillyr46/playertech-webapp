    ...
    template: `
        ...
        <div class="space-y-3">
            <div class="space-y-1">
                ...
                <p class="m-0 max-w-2xl text-sm text-slate-500 dark:text-slate-400">Administra el registro, el estado y la ubicación principal de cada academia.</p>
            </div>
                ...
                <div class="space-y-4">
                    <div class="rounded-[0.9rem] border border-slate-200 bg-slate-50/80 p-4 dark:border-surface-700 dark:bg-surface-900/40">
                        <div class="flex items-start justify-between gap-4">
                            <div class="min-w-0">
                                <p class="text-[0.68rem] font-semibold uppercase tracking-[0.22em] text-sky-700 dark:text-sky-400">Paso {{ currentStep + 1 }} de 3</p>
                            ...
                            <div class="hidden text-right text-xs leading-5 text-slate-500 dark:text-slate-400 md:block">
                                <div>Academia</div>
                                <div>Usuario inicial</div>
                                <div>Primer equipo</div>
                            </div>
                        </div>
                    ...
                    @if (currentStep === 0) {
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-12 flex flex-col gap-2">
                                <label for="name" class="text-sm font-medium text-surface-700 dark:text-surface-200">
                                    Nombre de la academia <span class="text-rose-500">*</span>
                                </label>
                                <input
                                    pInputText
                                    id="name"
                                    ...
                                    [(ngModel)]="form.name"
                                    name="name"
                                    placeholder="Ej. Academia PlayerTech"
                                    class="w-full"
                                    (keydown)="onRestrictedNameKeydown($event)"
                                    (paste)="onRestrictedNamePaste($event)"
                            ...
                            <div class="col-span-12 md:col-span-6 flex flex-col gap-2">
                                ...
                                <input
                                    ...
                                    name="contactEmail"
                                    placeholder="Ej. contacto@academia.com"
                                    class="w-full"
                            ...
                            <div class="col-span-12 md:col-span-6 flex flex-col gap-2">
                                <label for="country" class="text-sm font-medium text-surface-700 dark:text-surface-200">
                            ...
                            <div class="col-span-12 flex flex-col gap-2">
                                <label for="phoneNumber" class="text-sm font-medium text-surface-700 dark:text-surface-200">
                                ...
                                <div class="grid grid-cols-12 gap-3">
                                    ...
                                    <input
                                        ...
                                        class="col-span-12 md:col-span-8 w-full"
                            ...
                            <div class="col-span-12 md:col-span-6 flex flex-col gap-2">
                                <label for="department" class="text-sm font-medium text-surface-700 dark:text-surface-200">
                            ...
                            <div class="col-span-12 md:col-span-6 flex flex-col gap-2">
                                <label for="citySelect" class="text-sm font-medium text-surface-700 dark:text-surface-200">
                    ...
                    @if (currentStep === 1) {
                        <div class="grid grid-cols-12 gap-4">
                            <div class="col-span-12 md:col-span-6 flex flex-col gap-2">
                                <label for="adminName" class="text-sm font-medium text-surface-700 dark:text-surface-200">
                                    Nombre del administrador <span class="text-rose-500">*</span>
                                </label>
                                <input
                                    pInputText
                                    id="adminName"
                                    ...
                                    (keydown)="onRestrictedNameKeydown($event)"
                                    (paste)="onRestrictedNamePaste($event)"
                                    (input)="onAdminNameInput($event)"
                                />
                                @if (showError('adminName')) {
                                    <p-message severity="error" size="small">Escribe el nombre del administrador.</p-message>
                                }
                            </div>

                            <div class="col-span-12 md:col-span-6 flex flex-col gap-2">
                                <label for="adminEmail" class="text-sm font-medium text-surface-700 dark:text-surface-200">
                                    Correo del administrador <span class="text-rose-500">*</span>
                                </label>
                                <input
                                    pInputText
                                    id="adminEmail"
                    ...
                    @if (currentStep === 2) {
                        <div class="space-y-4">
                            <div class="grid grid-cols-12 gap-4">
                                <div class="col-span-12 md:col-span-6 flex flex-col gap-2">
                                    <label for="categoryId" class="text-sm font-medium text-surface-700 dark:text-surface-200">
                                        Categoría inicial <span class="text-rose-500">*</span>
                                    </label>
                                    <p-select
                                        id="categoryId"
                                ...
                                <div class="col-span-12 md:col-span-6 flex flex-col gap-2">
                                    <label for="teamName" class="text-sm font-medium text-surface-700 dark:text-surface-200">
                                        Nombre del primer equipo <span class="text-rose-500">*</span>
                                    </label>
                                    <input
                                        pInputText
                                        id="teamName"
                            ...
                            <div class="rounded-[0.9rem] border border-slate-200 bg-white px-4 py-4 shadow-sm dark:border-surface-700 dark:bg-surface-900">
                                ...
                                <div class="mt-4 grid gap-3 sm:grid-cols-3">
                                    ...
                                    <div class="rounded-[0.8rem] bg-slate-50 px-4 py-3 dark:bg-surface-800/60">
                                        <p class="text-xs uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">Administrador</p>
...
export class Tenants implements OnInit {
    ...
    editTenant(tenant: TenantListItem) {
        const form = this.tenantService.getById(tenant.id);
    ...
    onEmailPaste(event: ClipboardEvent) {
        ...
        if (pastedText !== sanitizedText) {
            ...
            if (input) {
                const field = input.name as 'contactEmail' | 'adminEmail' | '';
                const start = input.selectionStart ?? input.value.length;
                const end = input.selectionEnd ?? input.value.length;
                const nextValue = input.value.slice(0, start) + sanitizedText + input.value.slice(end);
                input.value = nextValue;

                if (field === 'contactEmail' || field === 'adminEmail') {
