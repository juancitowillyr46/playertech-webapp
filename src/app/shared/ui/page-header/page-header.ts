...
@Component({
    ...
    template: `
        <div class="rounded-[0.75rem] border border-slate-200 bg-white px-5 py-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)] dark:border-surface-800 dark:bg-surface-900">
            <div class="flex items-start justify-between gap-4">
                <div class="min-w-0 flex-1 space-y-0.5">
                    @if (breadcrumbs.length) {
                        <div class="flex flex-wrap items-center gap-2 text-[0.82rem] leading-none text-slate-500 dark:text-slate-400">
                    ...
                    <div class="my-1">
                        <p class="!m-0 text-[1.25rem] font-semibold leading-[1.2] text-surface-900 dark:text-surface-0">{{ title }}</p>
                    </div>
