...
export class AuthApiService {
    ...
    login(credentials: AuthCredentials): Observable<AuthUser> {
        return this.http.post<ApiEnvelope<AuthUser> | AuthUser>('/api/v1/auth/login', credentials).pipe(
    ...
    me(): Observable<AuthUser> {
        return this.http.get<ApiEnvelope<AuthUser> | AuthUser>('/api/v1/auth/me').pipe(
            map((response) => this.unwrapUser(response)),
            catchError((error) => throwError(() => this.normalizeError(error)))
        );
    }
    ...
    activateUser(token: string, payload: PasswordResetConfirm): Observable<AuthUser> {
        return this.http.post<ApiEnvelope<AuthUser> | AuthUser>(`/api/v1/public/users/activate/${encodeURIComponent(token)}`, payload).pipe(
            map((response) => this.unwrapUser(response)),
            catchError((error) => throwError(() => this.normalizeError(error)))
        );
    }
    ...
    confirmPasswordReset(token: string, payload: PasswordResetConfirm): Observable<void> {
        return this.http.post<void>(`/api/v1/public/users/password-reset/confirm/${encodeURIComponent(token)}`, payload).pipe(catchError((error) => throwError(() => this.normalizeError(error))));
    }

    updateName(fullName: string): Observable<AuthUser> {
        return this.http.put<ApiEnvelope<AuthUser> | AuthUser>('/api/v1/auth/me/name', { fullName }).pipe(
            map((response) => this.unwrapUser(response)),
            catchError((error) => throwError(() => this.normalizeError(error)))
        );
    }
